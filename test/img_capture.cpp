...
int cr_mkdir(const char *root_path) {
  struct stat info;
  if(stat(root_path, &info) != 0) {
    if(mkdir(root_path, 0777) != 0) {
      cout << "[CORERAIN] Error to make directory" << root_path << endl;
      return(-1);
    } else {
      return (0);
    }
  } else if(info.st_mode & S_IFDIR ) {
    cout << "[CORERAIN] Directory named " << root_path << " exists." << endl;
    char dtm_tmp;
    while(1) {
    ...
    cout << "[CORERAIN] File named " << root_path << " exists." << endl;
    return(-1);
  }
}

void set_ppl_info(string &name, string &gender, string &dob, string &ID_num) {
  bool finished = false;
  int option;
  name = "";
  gender = "";
...
int main(int argc, char** argv) {
  ...
  while(!finished) {
    ...
    set_ppl_info(name, gender, dob, ID_num);

    time_t now = time(0);
    string sub_path = to_string(now)+ "_" + name +"/";
    int loc = root_path.find_last_of("/\\");
    if(loc+1 != root_path.length()) {
      root_path += "/";
    }
    sub_path = root_path+sub_path;

    cr_mkdir(sub_path.c_str());
